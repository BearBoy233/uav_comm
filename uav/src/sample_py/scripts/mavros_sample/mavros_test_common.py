...
class MavrosTestCommon(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            rospy.wait_for_service('/car1/mavros/param/get', service_timeout)
            rospy.wait_for_service('/car1/mavros/param/set', service_timeout)
            rospy.wait_for_service('/car1/mavros/cmd/arming', service_timeout)
            rospy.wait_for_service('/car1/mavros/mission/push', service_timeout)
            rospy.wait_for_service('/car1/mavros/mission/clear', service_timeout)
            rospy.wait_for_service('/car1/mavros/set_mode', service_timeout)
            rospy.loginfo("ROS services are up")
        except rospy.ROSException:
            self.fail("failed to connect to services")
        self.get_param_srv = rospy.ServiceProxy('/car1/mavros/param/get', ParamGet)
        self.set_param_srv = rospy.ServiceProxy('/car1/mavros/param/set', ParamSet)
        self.set_arming_srv = rospy.ServiceProxy('/car1/mavros/cmd/arming',
                                                 CommandBool)
        self.set_mode_srv = rospy.ServiceProxy('/car1/mavros/set_mode', SetMode)
        self.wp_clear_srv = rospy.ServiceProxy('/car1/mavros/mission/clear',
                                               WaypointClear)
        self.wp_push_srv = rospy.ServiceProxy('/car1/mavros/mission/push',
                                              WaypointPush)

        # ROS subscribers
        self.alt_sub = rospy.Subscriber('/car1/mavros/altitude', Altitude,
                                        self.altitude_callback)
        self.ext_state_sub = rospy.Subscriber('/car1/mavros/extended_state',
                                              ExtendedState,
                                              self.extended_state_callback)
        self.global_pos_sub = rospy.Subscriber('/car1/mavros/global_position/global',
                                               NavSatFix,
                                               self.global_position_callback)
        self.imu_data_sub = rospy.Subscriber('/car1/mavros/imu/data',
                                               Imu,
                                               self.imu_data_callback)
        self.home_pos_sub = rospy.Subscriber('/car1/mavros/home_position/home',
                                             HomePosition,
                                             self.home_position_callback)
        self.local_pos_sub = rospy.Subscriber('/car1/mavros/local_position/pose',
                                              PoseStamped,
                                              self.local_position_callback)
        self.mission_wp_sub = rospy.Subscriber(
            '/car1/mavros/mission/waypoints', WaypointList, self.mission_wp_callback)
        self.state_sub = rospy.Subscriber('/car1/mavros/state', State,
                                          self.state_callback)
